...
namespace Dynamic_Smoke_System.Demo {
    public class DynamicSmokeSystemDemoMultipleController : MonoBehaviour {
        [SerializeField] private DynamicSmokeSystemController[] dynamicSmokeSystemControllers;
        [SerializeField] private Transform[] emissionTransforms;
        ...
        public void Emit() {
            for (var i = 0; i < dynamicSmokeSystemControllers.Length; i++) {
                var dynamicSmokeSystemController = dynamicSmokeSystemControllers[i];
                var position = emissionTransforms[i].position;
                dynamicSmokeSystemController.Emit(position);
            }
        }

        public void Disperse() {
            foreach (var dynamicSmokeSystemController in dynamicSmokeSystemControllers) {
